...
class SplitVideo(SplitVideoInterface):
    def __init__(self, no_of_bytes = 32, threshold = 20, output_path = 'video_scenes/', video_format = '.mp4'):
        self.video_name = str()
    ...
    def video_splits(self, local_video_path):
        ob = PySceneDetection(self.no_of_bytes, self.threshold, self.output_path, self.video_format)
        ob.detect_scenes(local_video_path)
        self.video_name = ob.get_video_name_from_filepath(local_video_path)
        local_file_path = self.output_path+self.video_name+'_split_times.csv'
        dataframe = pd.read_csv(local_file_path)
        for index, rows in dataframe.iterrows():
            self.best_splits(local_video_path, self.output_path + rows['filename']+self.video_format,\
                rows['Start Timecode'], rows['End Timecode'] )
